...
ADefHandler::openFile(const char* FilePath) {
	...
	if (inFile.good())
        m_bFileStream ->open(m_cFilePath, ios::in | ios::binary);
	else
		throw("Cannot Open File.");

	// store filepath&verify type/version
	this->m_iFileSize = getFileSize(inFile);
    fs = new std::istream(m_bFileStream );
	ValidateADEFs();
	if (!isValidFile) { return; }

	// reset-seek
	fs->seekg(0);
    m_bFileStream ->pubseekpos(ios_base::beg);
	LoadAnimDefData();

	// clean memory
	fs->clear();
    m_bFileStream ->close();
    if (m_bZlibStream  != nullptr) { m_bZlibStream ->clear(); delete m_bZlibStream ; }
	delete fs;
}
...
ADefHandler::LoadArguments(int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        switch (arguement.type) {
            ...
            case 0x4:
                size = ReadUInt32(*fs);
                arguement.name = ReadString(*fs, size); // string
                break;
            case 0x3:
                arguement.fValue = ReadFloat(*fs); // float
                break;
            case 0x2:
                arguement.value = ReadSInt32(*fs); // value; todo: check 32bit or 16bit;
                break;
            case 0x1:
                arguement.flag = ReadByte(*fs); // bool
                break;
            default:
                break;          }
...
ADefHandler::GetEventIDs(){
    ...
    /* Sort the vector of pairs based on the hash values */
    ...
    for (int i = 0; i < combinedVector.size(); i++){
        m_aEventTitles[i] = combinedVector[i].first;
        m_aEventHashes[i] = combinedVector[i].second;
    }
}
...
ADefHandler::LoadAnimDef(int size) {
	...
	fs->seekg(filePos);
	for (int i = 0; i < size; i++) {
        C_StateNode newStateNode(fs, this);
...
ADefHandler::LoadAnimDefData() {
	...
	uint32_t binType;
	uint32_t binCount;
    ProjectDefs cProjDefs(fs, &this->m_aEventTitles);
...
ADefHandler::decompressFile() {
    ...
    m_bZlibStream  = new std::stringstream;
    m_bZlibStream ->write((char*)data, dataSize);

	//clear remaining streams and set pointers
    m_bFileStream ->close(); fs->clear();
	if (data != nullptr) { this->isValidFile = true; }
	this->m_iFileSize = dataSize;
    fs->set_rdbuf(m_bZlibStream ->rdbuf());
}
